...
from schemas.graphs import ContractPresenceSummary, ContractType, PresenceItem
from schemas.presence_schemas import PRESENCE_SCHEMAS
...
def build_presence_summary(
    ...
    """
    根据 PRESENCE_SCHEMAS[contract_type] 和合同条款文本列表，生成统一的 ContractPresenceSummary。

    参数：
    - contract_type: 合同类型枚举，通常来自 ContractMeta.contract_type；
    - clauses: 条款原文列表（或者你现在内部使用的条款对象列表也可以，视实际结构调整）。

    注意：这里先保留一个简单实现，你可以后续把 detect_presence_for_item 替换为真正的规则/LLM 检测逻辑。
    """
    if isinstance(contract_type, ContractType):
        ct_key = contract_type.value
        contract_type_value = contract_type
    else:
        ct_key = str(contract_type)
        try:
            contract_type_value = ContractType(ct_key)
        except ValueError:
            contract_type_value = ContractType.OTHER

    schema = PRESENCE_SCHEMAS.get(ct_key, [])
    ...
    for item_def in schema:
        ...
        for clause in clauses:
            clause_types = clause.get("clause_type") or []
            if pid in clause_types:
                chunk_id = clause.get("chunk_id")
                if isinstance(chunk_id, str) and chunk_id:
                    clause_ids.append(chunk_id)

        present_flag = bool(clause_ids)
    ...
    return ContractPresenceSummary(
...
def process_file(
    ...
    presence_summary = build_presence_summary(business_type, chunk_records)
    ...
    doc_record = {
        ...
        "tags": tags,
        "presence_summary": presence_summary.model_dump(),
